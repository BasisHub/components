...
public class ResultSetImporter {
	...
	/**
	 * Parses the given Excel file and returns a com.basiscomponents.db.ResultSet
	 * with the data defined the the Excel file. <br/>
	 * <br/>
	 * <b>Note:</b> The first row of the Excel file must contain the column names,
	...
	/**
	 * Parses the given Excel file and returns a com.basiscomponents.db.ResultSet
	 * with the data defined the the Excel file. The method uses the given
	 * DataRow(Attributes record) to determine and set the correct field types in
	 * the ResultSet. <br/>
	 * <br/>
	 * <b>Note:</b> The first row of the Excel file must contain the column names,
	 * else an Exception gets thrown.
	 ...
	 *            The DataRow used to set the correct data types for the fields
	 ...
	 * with the data defined the the Excel file. The method uses the given
	 * DataRow(Attributes record) to determine and set the correct field types in
	 * the ResultSet. The given boolean value determines whether the first line
	 * contains the column headers or not. If it is set to true, the first row will
	 * be treated as normal row with field values and the given List will then be
	 * used to initialize the field names. If it is set to false, the first row will
	 * be used to initialize the field names.
	 * 
	 * @see #readExcel(File)
	 * @see #readExcel(File, DataRow)
	 * 
	 ...
	 *            The Excel File to parse.
	 * @param attributesRecord
	 *            he DataRow used to set the correct data types for the fields
	 *            defined in the Excel file.
	 * @param columnNames
	 *            The column names to use(Only required if the firstRowHasData
	 *            boolean is set to true).
	 * @param firstRowHasData
	 *            The boolean value used to determine if the first row contains the
	...
	public static ResultSet readExcel(File excelFile, DataRow attributesRecord, List<String> columnNames,
			Boolean firstRowHasData) throws Exception {
