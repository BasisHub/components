...
public class ResultSet implements java.io.Serializable, Iterable<DataRow> {
     ...
     *
     * @param rs
     * @param defaultMetaData If the defaultMetaData flag is set to true, the ResultSet will use the
     *                        metadata from the given java.sql.ResultSet object. If it is set to false, no
     *                        metadata will be created. If the resultSet already has some metadata defined,
     *                        it will not be removed.
     * @throws Exception
    ...
    public void add(DataRow dr) {
        ...
        if (isIndexed) {
            ...
            String idx = nameUUIDFromBytes(dr.toString().getBytes()).toString();
    ...
    public void createIndex() throws ParseException {
        if (!isIndexed()) {
            ...
            boolean hasIndexes = !indexColumns.isEmpty();
            for (DataRow dataRow : this) {
                String idx = dataRow.getRowKey();
                if (idx.isEmpty()) {
                    //TODO: if the ResultSet has a primary index, like from JDBC, use these fields only!
                    if (hasIndexes) {
                        String uuidSeed =indexColumns.stream().map(x->x+"="+dataRow.getDataField(x).toString()).collect(Collectors.joining(","));
                        idx = nameUUIDFromBytes(uuidSeed.getBytes()).toString();
                    } else {
                        idx = nameUUIDFromBytes(dataRow.toString().getBytes()).toString();
                    }
                    // workaround if there are true duplicate rows
                    while (this.rowIndex.containsKey(idx)) {
                        idx += '-';
                        idx += i;
                    }

                    dataRow.setRowKey(idx);
                }
                this.rowIndex.put(idx, i);
                i++;
            }
            this.isIndexed = true;
        }
