...
public class GeneratorJavaScript {
	...
	public static void createJavaScriptProxyClasses(HashMap<String, ParseEntity> classes, String outputfolder,
			String classfileprefix, String classfilesuffix) throws FileNotFoundException, UnsupportedEncodingException {
		...
		while (itfiles.hasNext()) {
			...
			try (PrintWriter writer = new PrintWriter(filename, "UTF-8")) {
				...
				if (pe.getClassname().contains("."))
					writer.println("	ses.create(this.id,\"" + pe.getClassname() + "\"); ");
				else
					writer.println("	ses.create(this.id,\"::" + classfileprefix + pe.getClassname() + classfilesuffix
							+ "::" + pe.getClassname() + "\"); ");
				writer.println("} ");
				...
				while (it1.hasNext()) {
					...
					if (m.getIsOverloaded()) {
						...
						writer.print(
								classname + ".prototype." + m.getName() + getMethodSignatureString(m) + " = function(");
					} else {
						writer.print(classname + ".prototype." + m.getName() + " = function(");
					}
					...
					// now invoke
					writer.print("	     ses.invoke(this.id," + rv_name + ",'" + m.getName() + "'");
					if (!m.getParams().isEmpty()) {
						writer.print(",args");
					}
					writer.println(");");
				...
				while (it.hasNext()) {
					...
					while (itm.hasNext()) {
						...
						if (m.getName().equals(methodname)) {
							...
							writer.println("        this." + methodname + getMethodSignatureString(m)
