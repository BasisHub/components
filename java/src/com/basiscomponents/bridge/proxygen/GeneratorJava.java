...
import com.basiscomponents.bridge.Session;
...
public class GeneratorJava {
	...
	public static void createJavaProxyClasses(HashMap<String, ParseEntity> classes,
			String packagename, String outputfolder, String classfileprefix, String classfilesuffix) throws FileNotFoundException, UnsupportedEncodingException {
	...
	while (it.hasNext()){
		...
		writer.println("		ses.create(classid,\"::"+classfileprefix+pe.getClassname()+classfilesuffix+"::"+pe.getClassname()+"\"); ");
		writer.println("	} ");
		writer.println("");
		...
		while (it1.hasNext()){
			...
			if (!OK) 
				continue;
			writer.print("	public "+(m.getReturnType()!=null ? m.getReturnType() : "void") +" "+m.getName()+ "(");
			...
			while (it2.hasNext()){
				MethodParameter mp = it2.next();
				writer.println("");				
				writer.println("	     //variable for parameter "+mp.getName());
				writer.println("	     String p_"+mp.getName()+"=  classid+\"_"+mp.getName()+"\";");
				writer.println("	     ses.pushVar(p_"+mp.getName()+", "+mp.getName()+");");
				writer.println("");				
			...
			{
				writer.println("	     String rv = classid+\"_"+m.getName()+"_retvar\";");
			...
			writer.println("	     ses.invoke(classid,rv,\""+m.getName()+"\""+invokeargs+");");
			writer.println("");					
			...
			if (!tmp.equals("void")){
					...
					writer.println("	     return ("+m.getReturnType()+")ses.getResult(rv);");
			}
			else{
