...
public class TestMyStuff {
	...
	public static void main(String[] args) throws Exception {
		...
		// String k =
		// r.replaceFields("        </tr>   pan=3 class=\"infoSubTitle\">$F{VISTADRZIP} $F{VISTADRCITY}</td>");
		...
		Date d = java.sql.Date.valueOf(com.basis.util.BasisDate.date(2457049,
				0.0, "%Yl-%Mz-%Dz"));
...
//		System.out.println(t1);
		 ...
		 * r.setFieldValue("DT", ""); // // r.setFieldValue("DT",-1); //
		 * System.out.println(r); //
		 * System.out.println(r.getFieldAsNumber("DT")); //
		 * System.out.println("-------------------------"); // //
		 ...
		 * r.setFieldValue("INTE",new java.sql.Date(
		 * com.basis.util.BasisDate.date(2457019).getTime() )); //
		 * r.setFieldValue("INTE",""); //
		 * System.out.println(r.getFieldAsString("INTE")); // //
		 * System.out.println(r.getFieldNames()); //
		 * System.out.println(r.clone().getFieldNames()); // // java.sql.Date d;
		 * // d=null; //// r.setFieldValue("testdate",d); // BBjNumber n ; // n=
		 * BasisNumber.createBasisNumber(-1); // //
		 ...
		 * System.out.println(r.getFieldAsNumber("testdate")); //
		 * System.out.println(r.getField("testdate")); // //
		 * r.setFieldValue("STR1","string1"); // // DataRow r1 = new DataRow();
		 * // r1.setFieldValue("STR1","string1a"); //
		 * r1.setFieldValue("STR2","string2a"); // // System.out.println(r); //
		 * System.out.println(r1); // r.mergeRecord(r1); //
		 * System.out.println(r); // // //
		 * Class.forName("com.basis.jdbc.BasisDriver"); // // String url =
		 * "jdbc:basis:beff8?DATABASE=CC&SSL=false"; // Connection con =
		 * DriverManager.getConnection(url, "admin", "admin123"); // Statement
		 ...
		 * ; // // //ResultSet rs =
		 * stmt.executeQuery("SELECT TOP 1 * FROM CLBASE "); // ResultSet rs =
		 * stmt.executeQuery(sql); // // // long
		 * start=System.currentTimeMillis(); // // while (rs.next()) // { //
		 * DataRow row = new DataRow(rs); //// System.out.println(row); // // }
